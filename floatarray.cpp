...
int main()
{
	...
	FloatList list;  // Fill in the code to define an object called list of the class FloatList
...
void FloatList::getList(ifstream& thing) {
	thing >> values[length];
	while (thing) {
		length++;
		thing.ignore();
...
FloatList::~FloatList() {
	...
	cout << "The average temperature is " << avg / length << endl;
}